...
export class ProductImportService implements OnApplicationBootstrap {
  ...
  constructor(
    ...
    private readonly options: ProductImportPluginOptions,
    ...
    this.jobQueue = await this.jobQueueService.createQueue({
      ...
      process: async (job) => {
        try {
          ...
          Logger.error(`Error importing products: ${e}`, loggerCtx);
        }
      },
    });
    if (this.options?.everyThisDay === undefined) {
      throw new UserInputError(
        `You need to configure ProductImportPluginOptions.everyThisOtherDay`
      );
    }
  ...
  async importProducts() {
    if (this.options?.url === undefined) {
      throw new UserInputError(
        `You need to configure "ProductImportPluginOptions.url"`
      );
    }
    ...
    const vendureProducts = await this.getExistingRemoteProducts(
      ctx,
      webHookIds
    );
    ...
    for (let remoteProduct of products) {
      const vendureProduct = vendureProducts.find(
        (vP) => vP.customFields.webhookId === remoteProduct.id
      );
      if (vendureProduct) {
  ...
  async updateProduct(
    ...
    let translation = vendureProduct.translations.find(
      (tr) => languageCode === tr.languageCode
    );
    const productTranslationRepo = this.connection.getRepository(
      ctx,
      ProductTranslation
    );
  ...
  async updateProductVariantName(
    ...
    let translation = defaultVariant.translations.find(
      (tr) => languageCode === tr.languageCode
    );
    const productTranslationRepo = this.connection.getRepository(
      ctx,
  ...
  async updateDefaultVariant(
    ...
    let defautVariant = vendureProduct.variants.find(
      (variant) => variant.sku === remoteProduct.sku
    );
    let defautVariantPriceInDefaultChannel =
      defautVariant?.productVariantPrices.find(
    ...
    const productRepo = this.connection.getRepository(ctx, Product);
    const productVariantPriceRepo = this.connection.getRepository(
      ctx,
      ProductVariantPrice
    );
    if (defautVariant && defautVariantPriceInDefaultChannel) {
      ...
      if (defautVariant?.productVariantPrices?.length) {
        defautVariant.productVariantPrices.push(
  ...
  async createProductVariant(
    ...
    const taxCategory =
      taxCategories.items.find((t) => t.isDefault === true) ??
  ...
  async updateProductCollection(
    ...
    let collection: Collection | null | undefined =
      defautVariant.collections?.find(
        (c) =>
          !!c.translations.find(
    ...
    if (!collection) {
      ...
      if (collection?.filters?.length) {
        ...
        if (variantIdFilter) {
          const variantIdsList: ID[] = JSON.parse(
            variantIdFilter.args[0].value
          );
  ...
  async updateProductFacet(
    ...
    let facetValue: FacetValue | null | undefined =
      vendureProduct.facetValues?.find(
        (facetValue) =>
    ...
    if (!facetValue) {
      ...
      if (facet) {
        ...
        facetValue = await this.facetValueService.create(
          ctx,
          facet,
          createFacetValueInput
        );
